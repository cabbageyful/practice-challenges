...
def parse(phrase, vocab):
    ...
    broken_phrases = set()
    # base case: phrase matches word in vocab
        # add to broken_phrases
    for word in vocab:
        if phrase == word:
            broken_phrases.add(phrase)

        elif phrase.startswith(word):
            remainder = phrase[len(word):]

            new_sentence = {word + ' ' + broken for broken in parse(remainder, vocab)}

            broken_phrases.update(new_sentence)
